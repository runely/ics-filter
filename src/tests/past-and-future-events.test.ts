...
const content: string = readFileSync("./ics/with-past-and-future-events.ics", "utf-8");
const expectedVEventCount: number = 2; // this one has 2 past events and 2 future events. we expect 2
const expectedVEventMaxCount: number = 1; // this one has 2 past events and 2 future events. we expect only 1
...
describe("icsFilter on ics content with past and future events with no max limit", () => {
...
describe("icsFilter on ics content with past and future events with max limit", () => {
  test("should always return ics content with a 'BEGIN:VCALENDAR' and a 'END:VCALENDAR'", () => {
